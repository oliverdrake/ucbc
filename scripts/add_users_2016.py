...
def create_user(first, last, email, password):
    ...
    user.save()
    print("New user: %s %s, u: %s e: %s, p: %s" % (
    first, last, username, email, password))
    return user


def create_email_address(user):
    return EmailAddress.objects.get_or_create(user=user, email=user.email,
                                              verified=True, primary=True)[0]
...
def deactivate_old_users():
    print(
    "deactivated %d users" % get_user_model().objects.filter(is_staff=False,
...
def main():
    ...
    with open('2016.csv', 'r') as csvfile:
        ...
        for fullname, _, student_id, email in reader:
